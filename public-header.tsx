...
export default function PublicHeader({ active = "" }: { active?: string }) {
  return <header className="nav-shell">
    ...
    <div className="nav-actions"><a className="whatsapp nav-cta" href={whatsapp} target="_blank" rel="noreferrer"><span>◉</span> {whatsappDisplay}</a><SocialLinks className="header-social-links" /><details className="mobile-services-menu"><summary aria-label="Abrir menú">☰</summary><div>{links.map((link) => <Link href={link.href} key={link.id}>{link.label}<span>→</span></Link>)}</div></details></div>
  </header>;
}
